    ...
    <div class="dashboard-container">
      ...
      <?php else: ?>
        <div class="media-grid">
          ...
          ?>
          <article class="media-item" data-id="<?= $fileId ?>" tabindex="0" aria-label="File <?= $fileName ?>">
            <div class="media-preview">
              <?php if (in_array($ext, ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp'])): ?>
                <img
                  ...
                  role="button"
                  tabindex="0"
                  onclick="openModal('<?= $filePath ?>', 'image')"
                  onkeypress="if(event.key==='Enter'){openModal('<?= $filePath ?>', 'image');}"
                >
              <?php elseif (in_array($ext, ['mp4', 'webm', 'ogg'])): ?>
                <video
                  controls
                  onclick="openModal('<?= $filePath ?>', 'video')"
                  onkeypress="if(event.key==='Enter'){openModal('<?= $filePath ?>', 'video');}"
                  tabindex="0"
                  role="button"
                >
              ...
              <?php else: ?>
                <div class="file-icon" tabindex="0" role="img" aria-label="File icon for <?= $fileName ?>">
            ...
            <div class="media-info">
              <p><i class="fas fa-file"></i> <span class="file-name"><?= $fileName ?></span></p>
              <p><i class="fas fa-hdd"></i> <?= $fileSize ?></p>
              <p><i class="fas fa-calendar-alt"></i> <?= $uploadedAt ?></p>
            </div>

            <div class="file-actions">
              <a href="<?= $filePath ?>" download="<?= $fileName ?>" class="btn" aria-label="Download <?= $fileName ?>">
                <i class="fas fa-download"></i> Download
              </a>
              <button class="btn btn-danger" onclick="confirmDelete(<?= $fileId ?>)" aria-label="Delete <?= $fileName ?>">
...
<div id="previewModal" class="modal" role="dialog" aria-modal="true" aria-labelledby="previewModalTitle">
  <button class="close" aria-label="Close preview" onclick="closeModal()">&times;</button>
...
<div id="deleteModal" class="modal" role="dialog" aria-modal="true" aria-labelledby="deleteModalTitle" aria-describedby="deleteModalDesc">
  <div class="modal-dialog">
    <h2 id="deleteModalTitle" class="modal-title"><i class="fas fa-exclamation-triangle"></i> Confirm Deletion</h2>
    <p id="deleteModalDesc">Are you sure you want to delete this file? This action cannot be undone.</p>
